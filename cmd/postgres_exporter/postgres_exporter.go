...
var (
	listenAddress          = kingpin.Flag("web.listen-address", "Address to listen on for web interface and telemetry.").Default(":9113").OverrideDefaultFromEnvar("PG_EXPORTER_WEB_LISTEN_ADDRESS").String()
...
func getDataSources() []string {
	...
	if len(dsn) == 0 {
		...
		if len(os.Getenv("DATA_SOURCE_DBNAME_FILE")) != 0 {
			fileContents, err := ioutil.ReadFile(os.Getenv("DATA_SOURCE_DBNAME_FILE"))
			if err != nil {
				panic(err)
			}
			dbname = strings.TrimSpace(string(fileContents))
		} else {
			dbname = os.Getenv("DATA_SOURCE_DBNAME")
		}

		ui := url.UserPassword(user, pass).String()
		if len(os.Getenv("DATA_SOURCE_SSL")) != 0 {
			dsn = "postgresql://" + ui + "@" + host + "/" + dbname
		} else {
			dsn = "postgresql://" + ui + "@" + host + "/" + dbname + "?sslmode=disable"
		}
